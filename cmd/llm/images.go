...
func imagesCmd(args []string) error {
	...
	for _, n := range slices.Sorted(maps.Keys(prompts)) {
		...
		if err := generate(key, body, prompts[n], dst); err != nil {
	...
	// ponytail: sequential; image rate limits are per-minute and low, so
	// parallelism mostly buys 429s, and skip-existing resume makes wall clock
	// a non-issue. Add a small worker pool if a paid tier makes it worth it.
