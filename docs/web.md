...
**Every spell has an icon**, a 128px webp under `src/assets/spells/<slug>.webp`,
...
They are *imported* (an `import.meta.glob` in `features/spells/spellIcon.tsx`)
rather than dropped in `public/`, for the same reason as the landing images:
Vite content-hashes them so nginx's immutable rule applies -- and being webp
they dodge the service worker's `png` precache glob, so 319 icons do not ride
along with every install. A slug with no file simply renders no icon. The set
is generated by `make spell-icons` -- art, not derived data, so there is
deliberately no drift check; rerolling one means deleting its webp and its
